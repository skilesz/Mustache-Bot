...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    if (!shop) {
      console.log(`WARNING (completeOrder.js): Shop does not exist`);
      return;
    }
    ...
    try {
      ...
      if (!order) {
        console.log(`WARNING (completeOrder.js): Order does not exist`);
        return;
      }
      ...
      await order.save().catch((e) => {
        console.log(`ERROR (completeOrder.js: save()): ${e}`);
      });

    } catch (err) {
      console.log(`ERROR (completeOrder.js): ${err}`);
    }
  }
};